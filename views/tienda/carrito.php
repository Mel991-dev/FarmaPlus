...
<div class="max-w-5xl mx-auto">
    ...
    <div class="flex items-center justify-between mb-8">
        <div>
            ...
            <p class="text-[13px] text-slate-500 mt-1">
                <?= $totalItems ?> producto
                <?= $totalItems !== 1 ? 's' : '' ?> en tu carrito
            </p>
        </div>
        <a href="<?= $basePath ?>/tienda"
            class="flex items-center gap-1.5 text-[13px] font-semibold text-fp-primary hover:underline">
    ...
    <?php if (empty($carrito)): ?>
        ...
        <div
            ...
            <p class="text-[14px] text-slate-500 max-w-xs mb-6">Agrega medicamentos y productos farmacéuticos desde nuestro
                catálogo.</p>
            <a href="<?= $basePath ?>/tienda"
                class="px-6 py-2.5 bg-fp-secondary text-white text-[14px] font-bold rounded-xl hover:bg-fp-secondary/90 transition-colors shadow-sm flex items-center gap-2">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-2 flex flex-col gap-3">
                <?php foreach ($carrito as $productoId => $item): ?>
                    <div class="bg-white rounded-xl border border-slate-200 p-4 flex flex-col sm:flex-row sm:items-center gap-4 cart-row"
                        ...
                        <div
                            class="w-16 h-16 rounded-xl bg-slate-50 border border-slate-200 overflow-hidden shrink-0 flex items-center justify-center">
                            <?php if (!empty($item['imagen'])): ?>
                                <img src="<?= $basePath ?>/assets/uploads/productos/<?= $productoId ?>/<?= htmlspecialchars($item['imagen']) ?>"
                                    alt="<?= htmlspecialchars($item['nombre']) ?>" class="w-full h-full object-cover" />
                            <?php else: ?>
                        ...
                        <div class="flex-1 min-w-0">
                            <div class="font-bold text-[14px] text-slate-800 leading-tight truncate">
                                <?= htmlspecialchars($item['nombre']) ?>
                            </div>
                            <div class="text-[13px] text-slate-500 mt-0.5">$
                                <?= number_format($item['precio_unitario'], 0, ',', '.') ?> / unidad
                            </div>
                        </div>
                        ...
                        <div class="flex items-center gap-2 shrink-0">
                            ...
                            <span class="w-8 text-center font-bold text-[14px] text-slate-800 qty-label"
                                id="qty-<?= $productoId ?>">
                                <?= $item['cantidad'] ?>
                            </span>
                            <button onclick="cambiarCantidad(<?= $productoId ?>, 1)"
                        ...
                        <div class="text-right shrink-0 sm:ml-auto">
                            ...
                            <button onclick="eliminarItem(<?= $productoId ?>)"
                ...
                <button onclick="vaciarCarrito()"
                    class="self-start text-[12px] text-slate-400 hover:text-fp-error transition-colors font-semibold flex items-center gap-1 mt-1">
            ...
            <div class="lg:col-span-1">
                <div class="bg-white rounded-2xl border border-slate-200 p-5 sticky top-24 shadow-sm">
                    <h2 class="font-bold text-[16px] text-slate-800 mb-4 pb-3 border-b border-slate-100">Resumen del pedido
                    </h2>

                    <div class="flex items-center justify-between mb-2">
                        <span class="text-[13px] text-slate-500">Subtotal (<span id="totalItemsLabel">
                                <?= $totalItems ?>
                            </span> items)</span>
                        <span class="font-semibold text-[14px] text-slate-800" id="subtotalLabel">$
                    ...
                    <div class="border-t border-slate-100 pt-4 mb-5">
                        <div class="flex items-center justify-between">
                            ...
                            <span class="font-black text-[17px] text-fp-primary" id="totalLabel">$
                                <?= number_format($subtotal, 0, ',', '.') ?>
                            </span>
                        </div>
                    ...
                    <div class="flex items-center justify-center gap-3 mt-4 text-[11px] text-slate-400">
                        <span class="flex items-center gap-1"><i data-lucide="shield-check"
                                class="w-3.5 h-3.5 text-green-500"></i> Pago seguro</span>
                        <span class="flex items-center gap-1"><i data-lucide="lock" class="w-3.5 h-3.5 text-blue-500"></i>
                            SSL</span>
                    </div>
                </div>
            </div>
