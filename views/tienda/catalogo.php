...
<!-- Hero Tienda -->
<div
    class="relative overflow-hidden rounded-2xl bg-gradient-to-br from-fp-primary via-[#0f4c65] to-[#1a6b8a] p-6 sm:p-8 mb-8">
    <div class="relative z-10">
        <div
            class="inline-flex items-center gap-2 bg-white/10 border border-white/20 text-white text-xs font-bold px-3 py-1 rounded-full mb-4">
        ...
        <p class="text-white/80 text-[14px] max-w-md">Medicamentos de venta libre, vitaminas y más. Entrega a domicilio
            rápida y segura.</p>
    </div>
    <!-- Decoración -->
...
<?php if (!empty($categorias)): ?>
    <div class="flex items-center gap-2 mb-6 overflow-x-auto no-scrollbar pb-1 scroll-smooth snap-x">
        <a href="<?= $basePath ?>/tienda"
            class="snap-start shrink-0 px-3 py-1.5 rounded-lg text-[13px] font-semibold transition-colors <?= empty($_GET['categoria']) ? 'bg-fp-primary text-white shadow-sm' : 'bg-white border border-slate-200 text-slate-600 hover:border-fp-primary hover:text-fp-primary' ?>">
            Todos
        </a>
...
<div class="flex items-center justify-between mb-4">
    <p class="text-[13px] text-slate-500 font-medium">
        ...
        encontrado<?= count($productos) !== 1 ? 's' : '' ?>
    </p>
    <?php if (!empty($_GET['q'])): ?>
        <div class="flex items-center gap-2">
            <span class="text-[12px] text-slate-500">Buscando: <strong><?= htmlspecialchars($_GET['q']) ?></strong></span>
            <a href="<?= $basePath ?>/tienda"
                class="text-[12px] text-fp-error hover:underline font-semibold flex items-center gap-1">
...
<?php if (empty($productos)): ?>
    <div class="flex flex-col items-center justify-center py-20 text-center">
        ...
        <p class="text-[13px] text-slate-500 max-w-xs mb-4">Intenta con términos diferentes o explora todas las categorías.
        </p>
        <a href="<?= $basePath ?>/tienda"
            class="px-4 py-2 bg-fp-primary text-white text-[13px] font-bold rounded-xl hover:bg-fp-primary-dark transition-colors">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-5">
        <?php foreach ($productos as $p): ?>
            ...
            <div class="bg-white rounded-2xl border border-slate-200 overflow-hidden flex flex-col shadow-sm hover:shadow-md hover:border-fp-primary/30 transition-all group"
                data-producto-id="<?= $p['producto_id'] ?>" data-nombre="<?= strtolower(htmlspecialchars($p['nombre'])) ?>"
                data-categoria="<?= (int) $p['categoria_id'] ?>">

                <!-- Imagen / placeholder — clickeable hacia la ficha del producto -->
                <a href="<?= $basePath ?>/tienda/producto/<?= $p['producto_id'] ?>"
                    class="block relative bg-gradient-to-br from-slate-50 to-slate-100 h-[160px] flex items-center justify-center overflow-hidden">
                    ...
                    <?php else: ?>
                        ...
                        <i data-lucide="<?= $iconoPlaceholder ?>"
                            class="w-16 h-16 text-fp-primary/20 group-hover:scale-110 group-hover:text-fp-primary/30 transition-all duration-300"></i>
                    <?php endif; ?>
                    <!-- Categoria badge -->
                    <span
                        class="absolute top-2.5 left-2.5 bg-white/90 backdrop-blur-sm text-[10px] font-bold uppercase tracking-wider text-fp-primary px-2 py-0.5 rounded-full border border-fp-primary/10">
                        <?= htmlspecialchars($p['categoria_nombre'] ?? '') ?>
                    </span>
                    <!-- Stock low badge -->
                    <?php if ($stockLow): ?>
                        <span
                            class="absolute top-2.5 right-2.5 bg-fp-warning/10 text-fp-warning text-[10px] font-bold px-2 py-0.5 rounded-full border border-fp-warning/20">
                            Últimas unidades
                        </span>
                    <?php endif; ?>
                </a>
                ...
                <div class="p-4 flex flex-col flex-1">
                    <h3 class="font-bold text-[14px] text-slate-800 leading-snug mb-1 line-clamp-2"
                    ...
                    <?php if (!empty($p['principio_activo'])): ?>
                        <p class="text-[11px] text-slate-500 font-medium mb-3 line-clamp-1">
                            <?= htmlspecialchars($p['principio_activo']) ?></p>
                    <?php endif; ?>
                    ...
                    <div class="flex items-center justify-between mt-auto">
                        ...
                        <div
                            class="flex items-center gap-1 text-[11px] text-slate-500 font-medium bg-slate-50 px-2 py-1 rounded-lg border border-slate-200">
