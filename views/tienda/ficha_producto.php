...
<div class="max-w-5xl mx-auto">
    ...
    <!-- Producto grid -->
    <div
        class="grid grid-cols-1 md:grid-cols-2 gap-8 bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">

        <!-- Galería de imágenes -->
        <div
            class="relative bg-gradient-to-br from-slate-50 to-slate-100 flex flex-col items-center justify-center min-h-[280px] md:min-h-[360px] gap-3 p-4">

            <!-- Imagen principal destacada -->
            <div class="w-full flex-1 flex items-center justify-center overflow-hidden rounded-xl"
                id="imgPrincipalWrap">
                <?php if ($imgPrincipal): ?>
                    <img id="imgDestacada"
            ...
            <?php if (!empty($producto['categoria_nombre'])): ?>
                <span
                    class="absolute top-4 left-4 bg-white/90 text-fp-primary text-[11px] font-bold uppercase tracking-wider px-3 py-1 rounded-full border border-fp-primary/10 shadow-sm">
        ...
        <div class="p-6 md:p-8 flex flex-col gap-5">
            ...
            <div>
                ...
                <?php if (!empty($producto['principio_activo'])): ?>
                    <p class="text-[13px] text-slate-500 font-medium">
                        Principio activo: <span class="font-mono text-fp-primary">
                            <?= htmlspecialchars($producto['principio_activo']) ?>
                        </span>
                    </p>
                <?php endif; ?>
            </div>
            ...
            <div class="flex items-center gap-2">
                ...
                <?php elseif ($stockActual > 0): ?>
                    ...
                    <span class="text-[13px] text-amber-600 font-semibold">Pocas unidades (
                        <?= $stockActual ?> disp.)
                    </span>
                <?php else: ?>
            ...
            <div class="grid grid-cols-2 gap-3 text-[13px]">
                <?php if (!empty($producto['laboratorio'])): ?>
                    <div class="bg-slate-50 rounded-xl p-3 border border-slate-100">
                        ...
                        <span class="font-semibold text-slate-700">
                            <?= htmlspecialchars($producto['laboratorio']) ?>
                        </span>
                    </div>
                <?php endif; ?>
                <?php if (!empty($producto['registro_invima'])): ?>
                    <div class="bg-slate-50 rounded-xl p-3 border border-slate-100">
                        ...
                        <span class="font-mono font-semibold text-fp-primary text-[12px]">
                            <?= htmlspecialchars($producto['registro_invima']) ?>
                        </span>
                    </div>
                <?php endif; ?>
            </div>
            ...
            <?php if ($hayStock): ?>
                <div class="flex items-center gap-3 mt-auto pt-4 border-t border-slate-100">
                    ...
                    <div id="qtyControls"
                        class="flex items-center gap-2 bg-slate-50 rounded-xl border border-slate-200 p-1">
                        <button type="button" id="btnMinus"
                            class="w-8 h-8 rounded-lg flex items-center justify-center text-slate-500 hover:bg-white hover:text-fp-primary transition-all disabled:opacity-30"
                            <?= $enCarrito <= 0 ? 'disabled' : '' ?>>
                            <i data-lucide="minus" class="w-4 h-4"></i>
                        </button>
                        <span id="qtyDisplay" class="w-8 text-center font-black text-slate-700 text-[15px]">
                            <?= max(1, $enCarrito) ?>
                        </span>
                        <button type="button" id="btnPlus"
                            class="w-8 h-8 rounded-lg flex items-center justify-center text-slate-500 hover:bg-white hover:text-fp-primary transition-all">
                    ...
                    <button id="btnAgregar"
                        ...
                        data-nombre="<?= htmlspecialchars($producto['nombre']) ?>" data-precio="<?= $precio ?>">
                ...
                <?php if ($enCarrito > 0): ?>
                    <a href="<?= $basePath ?>/tienda/carrito"
                        class="flex items-center justify-center gap-2 text-[13px] text-fp-primary hover:underline font-semibold">
                        <i data-lucide="arrow-right" class="w-4 h-4"></i> Ver carrito (
                        <?= $enCarrito ?> unidades)
                    </a>
                <?php endif; ?>
            ...
            <?php else: ?>
                <div
                    class="bg-slate-50 rounded-xl p-4 border border-slate-200 text-center text-[13px] text-slate-500 mt-auto">
    ...
    <div class="mt-6">
        <a href="<?= $basePath ?>/tienda"
            class="inline-flex items-center gap-2 text-[13px] text-slate-500 hover:text-fp-primary transition-colors font-medium">
