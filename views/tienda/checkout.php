...
<div class="max-w-5xl mx-auto">
    ...
    <div class="flex items-center gap-2 sm:gap-4 mb-8 overflow-x-auto no-scrollbar pb-1">
        <div class="flex items-center gap-2 shrink-0">
            <div
                class="w-7 h-7 rounded-full bg-fp-success text-white flex items-center justify-center text-[12px] font-bold shrink-0">
        ...
        <div class="flex items-center gap-2 shrink-0">
            <div
                class="w-7 h-7 rounded-full bg-fp-primary text-white flex items-center justify-center text-[12px] font-bold shrink-0">
                2</div>
        ...
        <div class="flex items-center gap-2 shrink-0">
            <div
                class="w-7 h-7 rounded-full bg-slate-200 text-slate-400 flex items-center justify-center text-[12px] font-bold shrink-0">
                3</div>
    ...
    <?php if (!empty($_GET['error'])): ?>
        <div
            class="mb-6 bg-fp-error/10 border-l-4 border-fp-error p-3 rounded-lg flex items-center gap-2 text-sm font-semibold text-fp-error">
            ...
            <div class="lg:col-span-2 flex flex-col gap-5">
                ...
                <div class="bg-white rounded-2xl border border-slate-200 p-5 shadow-sm">
                    ...
                    <?php if (empty($direcciones)): ?>
                        <div
                            class="flex flex-col items-center p-6 bg-slate-50 rounded-xl border border-dashed border-slate-300 text-center">
                            ...
                            <p class="text-[13px] text-slate-500 mb-4">Agrega una dirección para continuar con el pedido.
                            </p>
                            <a href="<?= $basePath ?>/mi-cuenta/direcciones"
                                class="px-4 py-2 bg-fp-primary text-white text-[13px] font-bold rounded-xl hover:bg-fp-primary-dark transition-colors flex items-center gap-2">
                    ...
                    <?php else: ?>
                        <div class="flex flex-col gap-3">
                            <?php foreach ($direcciones as $idx => $dir): ?>
                                <label
                                    class="flex items-start gap-3 p-4 rounded-xl border-2 cursor-pointer transition-all hover:border-fp-primary/50 has-[:checked]:border-fp-primary has-[:checked]:bg-fp-primary/5 group"
                                    for="dir-<?= $dir['direccion_id'] ?>">
                                    <input type="radio" name="direccion_entrega_id" id="dir-<?= $dir['direccion_id'] ?>"
                                        value="<?= $dir['direccion_id'] ?>" <?= $idx === 0 ? 'checked' : '' ?>
                                        class="mt-0.5 accent-fp-primary w-4 h-4 shrink-0" required>
                                    <div class="flex-1 min-w-0">
                                        <div class="font-bold text-[14px] text-slate-800">
                                            <?= htmlspecialchars($dir['direccion']) ?></div>
                                        <div class="text-[12px] text-slate-500 mt-0.5">
                                            <?php if (!empty($dir['barrio'])): ?>            <?= htmlspecialchars($dir['barrio']) ?>,
                                            <?php endif; ?>
                                            <?= htmlspecialchars($dir['ciudad'] ?? '') ?>
                                        </div>
                                        <?php if (!empty($dir['referencia'])): ?>
                                            <div class="text-[11px] text-slate-400 mt-1 italic">
                                                <?= htmlspecialchars($dir['referencia']) ?></div>
                                        <?php endif; ?>
                                    </div>
                                    <?php if ($dir['principal'] ?? false): ?>
                                        <span
                                            class="shrink-0 text-[10px] font-bold text-fp-primary bg-fp-primary/10 px-2 py-0.5 rounded-full">Principal</span>
                                    <?php endif; ?>
                                </label>
                            <?php endforeach; ?>
                        </div>
                        <a href="<?= $basePath ?>/mi-cuenta/direcciones"
                            class="mt-3 text-[12px] text-fp-primary hover:underline font-semibold flex items-center gap-1">
                ...
                <div class="bg-white rounded-2xl border border-slate-200 p-5 shadow-sm">
                    ...
                    <div class="flex flex-col sm:flex-row gap-3">
                        <!-- MercadoPago -->
                        <label
                            class="flex items-center gap-4 p-4 flex-1 rounded-xl border-2 border-fp-primary bg-fp-primary/5 cursor-pointer">
                            <input type="radio" name="metodo_pago" value="mercadopago" checked
                                class="accent-fp-primary w-4 h-4 shrink-0">
                            <div class="flex-1">
                                <div class="font-bold text-[14px] text-slate-800 flex items-center gap-2">
                                    <span
                                        class="text-[11px] px-2 py-0.5 bg-[#009EE3] text-white rounded font-bold">MP</span>
                                    MercadoPago
                                </div>
                ...
                <div class="bg-white rounded-2xl border border-slate-200 p-5 sticky top-24 shadow-sm">
                    ...
                    <div class="flex flex-col gap-2 mb-4">
                        <?php foreach ($carrito as $item): ?>
                            <div class="flex items-start justify-between gap-2">
                                <div class="flex-1 min-w-0">
                                    <div class="text-[12px] font-semibold text-slate-700 leading-tight truncate">
                                        <?= htmlspecialchars($item['nombre']) ?></div>
                                    <div class="text-[11px] text-slate-400">x<?= $item['cantidad'] ?></div>
                                </div>
                                <span
                                    class="text-[13px] font-bold text-slate-700 shrink-0">$<?= number_format($item['precio_unitario'] * $item['cantidad'], 0, ',', '.') ?></span>
                            </div>
                        <?php endforeach; ?>
                    </div>
                    ...
                    <div class="border-t border-slate-100 pt-3 flex flex-col gap-1.5 mb-4">
                        ...
                        <div class="flex justify-between text-[13px]">
                            ...
                            <span
                                class="font-semibold text-fp-success">$<?= number_format($costoEnvio, 0, ',', '.') ?></span>
                        </div>
                        <div class="border-t border-slate-100 mt-2 pt-2 flex justify-between">
                            <span class="font-black text-[15px]">Total</span>
                            <span
                                class="font-black text-[17px] text-fp-primary">$<?= number_format($total, 0, ',', '.') ?></span>
                        </div>
                    </div>
                    ...
                    <?php if (!empty($direcciones)): ?>
                        ...
                        <p class="text-[11px] text-slate-400 text-center mt-2">Serás redirigido al checkout seguro de
                            MercadoPago</p>
                    <?php else: ?>
                        <div
                            class="w-full h-12 flex items-center justify-center bg-slate-100 text-slate-400 text-[13px] font-semibold rounded-xl cursor-not-allowed">
