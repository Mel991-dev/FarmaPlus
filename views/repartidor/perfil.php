        ...
        <div class="w-full lg:w-1/3 flex flex-col gap-5">
            <div class="bg-white rounded-xl border border-fp-border shadow-sm p-6 flex flex-col items-center text-center">
                ...
                <div class="w-24 h-24 rounded-full bg-fp-secondary flex items-center justify-center text-3xl font-bold text-white shadow-md mb-4">
                    <?= strtoupper(substr($usuario['nombres'] ?? 'R', 0, 1) . substr($usuario['apellidos'] ?? 'P', 0, 1)) ?>
                </div>

                <h3 class="text-xl font-bold text-fp-text leading-tight"><?= htmlspecialchars($usuario['nombres'] ?? '') ?> <?= htmlspecialchars($usuario['apellidos'] ?? '') ?></h3>
                <p class="text-fp-muted text-sm mt-1">Repartidor</p>
                ...
                <div class="w-full flex flex-col gap-4 text-left">
                    <div class="flex items-start gap-3">
                        ...
                        <div>
                            ...
                            <p class="text-sm font-medium text-fp-text font-mono"><?= htmlspecialchars($usuario['documento'] ?? 'No registrado') ?></p>
                        </div>
                    </div>
                    ...
                    <div class="flex items-start gap-3">
                        ...
                        <div>
                            ...
                            <p class="text-sm font-medium text-fp-text break-all"><?= htmlspecialchars($usuario['correo'] ?? '') ?></p>
                        </div>
                    </div>
                    ...
                    <div class="flex items-start gap-3">
                        ...
                        <div>
                            ...
                            <p class="text-sm font-medium text-fp-text"><?= htmlspecialchars($usuario['telefono'] ?? 'No registrado') ?></p>
                        </div>
                    </div>
                    ...
                    <div class="flex items-start gap-3">
                        ...
                        <div>
                            ...
                            <p class="text-sm font-medium text-fp-text"><?= date('d M Y, H:i', strtotime($usuario['created_at'] ?? 'now')) ?></p>
                        </div>
                    </div>
                </div>
        ...
        <div class="w-full lg:w-2/3">
            <div class="bg-white rounded-xl border border-fp-border shadow-sm flex flex-col h-full overflow-hidden">
                ...
                <div class="p-6 border-b border-fp-border bg-fp-bg-main/30">
                    ...
                    <p class="text-sm text-fp-muted mt-1">Actualiza tu información de contacto personal.</p>
                </div>

                <form method="POST" action="<?= rtrim($_ENV['APP_BASEPATH'] ?? '', '/') ?>/repartidor/perfil/actualizar" class="p-6 flex flex-col gap-5 flex-1">
                    
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                        <div class="flex flex-col gap-1.5">
                            <label for="nombres" class="text-sm font-semibold text-fp-text">Nombres <span class="text-fp-error">*</span></label>
                            <input type="text" id="nombres" name="nombres" value="<?= htmlspecialchars($usuario['nombres'] ?? '') ?>" required class="w-full px-4 py-2.5 bg-fp-bg-main border border-fp-border rounded-lg text-sm text-fp-text focus:outline-none focus:border-fp-primary focus:ring-2 focus:ring-fp-primary/20 transition-all">
                        </div>
                        <div class="flex flex-col gap-1.5">
                            <label for="apellidos" class="text-sm font-semibold text-fp-text">Apellidos <span class="text-fp-error">*</span></label>
                            <input type="text" id="apellidos" name="apellidos" value="<?= htmlspecialchars($usuario['apellidos'] ?? '') ?>" required class="w-full px-4 py-2.5 bg-fp-bg-main border border-fp-border rounded-lg text-sm text-fp-text focus:outline-none focus:border-fp-primary focus:ring-2 focus:ring-fp-primary/20 transition-all">
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                        <div class="flex flex-col gap-1.5">
                            ...
                            <input type="email" id="correo" name="correo" value="<?= htmlspecialchars($usuario['correo'] ?? '') ?>" required class="w-full px-4 py-2.5 bg-fp-bg-main border border-fp-border rounded-lg text-sm text-fp-text focus:outline-none focus:border-fp-primary focus:ring-2 focus:ring-fp-primary/20 transition-all">
                        </div>
                        <div class="flex flex-col gap-1.5">
                            <label for="telefono" class="text-sm font-semibold text-fp-text">Teléfono Celular</label>
                            <input type="tel" id="telefono" name="telefono" value="<?= htmlspecialchars($usuario['telefono'] ?? '') ?>" class="w-full px-4 py-2.5 bg-fp-bg-main border border-fp-border rounded-lg text-sm text-fp-text focus:outline-none focus:border-fp-primary focus:ring-2 focus:ring-fp-primary/20 transition-all">
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                        <div class="flex flex-col gap-1.5">
                            ...
                            <input type="text" id="documento" value="<?= htmlspecialchars($usuario['documento'] ?? '') ?>" disabled class="w-full px-4 py-2.5 bg-[#f1f5f9] border border-fp-border rounded-lg text-sm text-fp-muted cursor-not-allowed font-mono">
...
<?php include __DIR__ . '/../layouts/base_repartidor.php'; ?>
